...
import { unpackFrom, pack, parseTagName, BitValue, BitofWord, getBitOfWord, getWordCount, parseIdentityResponse, nameFunction } from "./utils";
import { RegisterSessionError, ForwarOpenError, getErrorCode, LogixError, ConnectionLostError, ConnectionTimeoutError, ConnectionError } from "./errors";
...
var EIPContext = (function (_super) {
    ...
    EIPContext.prototype.on = function (event, listener) {
        ...
        return nameFunction("off_" + event, function () {
...
var EIPSocket = (function (_super) {
    ...
    EIPSocket.prototype.buildRegisterSession = function () {
        return pack("<HHIIQIHH", 0x0065, 0x0004, this.sessionHandle, 0x0000, this._context, 0x0000, 0x01, 0x00);
    };
    EIPSocket.prototype.buildUnregisterSession = function () {
        return pack("<HHIIQI", 0x66, 0x0, this.sessionHandle, 0x0000, this._context, 0x0000);
    };
    ...
    EIPSocket.prototype.buildCIPForwardOpen = function () {
        ...
        var ForwardOpen = pack(pack_format, CIPService, 0x02, 0x20, 0x06, 0x24, 0x01, 0x0a, 0x0e, 0x20000002, 0x20000001, this.serialNumber, this.context.vendorId, this.originatorSerialNumber, 0x03, 0x00201234, CIPConnectionParameters, 0x00204001, CIPConnectionParameters, 0xa3);
        pack_format = "<B" + this.context.connectionPath.length + "B";
        var data = pack.apply(void 0, __spreadArrays([pack_format,
    ...
    EIPSocket.prototype.buildForwardClose = function () {
        var ForwardClose = pack("<BBBBBBBBHHI", 0x4e, 0x02, 0x20, 0x06, 0x24, 0x01, 0x0a, 0x0e, this.serialNumber, this.context.vendorId, this.originatorSerialNumber);
        var pack_format = "<H" + this.context.connectionPath.length + "B";
        var CIPConnectionPath = pack.apply(void 0, __spreadArrays([pack_format,
            this.context.connectionPathSize], this.context.connectionPath));
    ...
    EIPSocket.prototype.buildEIPSendRRDataHeader = function (frameLen) {
        return pack("<HHIIQIIHHHHHH", 0x6f, 16 + frameLen, this.sessionHandle, 0x00, this._context, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0xb2, frameLen);
    };
    EIPSocket.prototype.buildEIPHeader = function (tagIOI) {
        if (this.contextPointer === 155)
            this.contextPointer = 0;
        this.contextPointer += 1;
        var EIPHeaderFrame = pack("<HHIIQIIHHHHIHHH", 0x70, 22 + tagIOI.length, this.sessionHandle, 0x00, context_dict[this.contextPointer], 0x0000, 0x00, 0x00, 0x02, 0xa1, 0x04, this.id, 0xb1, tagIOI.length + 2, this.sequenceCounter);
    ...
    EIPSocket.prototype.buildMultiServiceHeader = function () {
        return pack("<BBBBBB", 0x0a, 0x02, 0x20, 0x02, 0x24, 0x01);
    };
    ...
    EIPSocket.prototype.addPartialReadIOI = function (tagIOI, elements) {
        var data1 = pack("<BB", 0x52, ~~(tagIOI.length / 2));
        var data2 = pack("<H", elements);
        var data3 = pack("<I", this.offset);
    ...
    EIPSocket.prototype.addReadIOI = function (tagIOI, elements) {
        var data1 = pack("<BB", 0x4c, ~~(tagIOI.length / 2));
        var data2 = pack("<H", elements);
    ...
    EIPSocket.prototype.buildTagIOI = function (tagName, isBoolArray) {
        ...
        tagArray.forEach(function (_tag, i) {
            if (_tag.endsWith("]")) {
                var _a = parseTagName(_tag, 0), _ = _a[0], basetag = _a[1], index = _a[2];
                var BaseTagLenBytes = basetag.length;
                if (isBoolArray &&
                    i === tagArray.length - 1 &&
                    typeof index === "number")
                    index = ~~(index / 32);
                var data1 = pack("<BB", 0x91, BaseTagLenBytes);
                ...
                if (i < tagArray.length) {
                    if (!Array.isArray(index)) {
                        ...
                        index.forEach(function (element) {
                            if (element < 256) {
                                var data = pack("<BB", 0x28, element);
                                RequestTagData = Buffer.concat([RequestTagData, data], data.length + RequestTagData.length);
                            }
                            if (65536 > element && element > 255) {
                                var data = pack("<HH", 0x29, element);
                                RequestTagData = Buffer.concat([RequestTagData, data], data.length + RequestTagData.length);
                            }
                            if (element > 65535) {
                                var data = pack("<HI", 0x2a, element);
                                RequestTagData = Buffer.concat([RequestTagData, data], data.length + RequestTagData.length);
                            }
                        });
                    }
    ...
    EIPSocket.prototype.buildCIPUnconnectedSend = function () {
        return pack("<BBBBBBBBH", 0x52, 0x02, 0x20, 0x06, 0x24, 0x01, 0x0a, 0x0e, 0x06);
    };
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 2:
                        ...
                        wordCount = getWordCount(index, elements, bitCount);
                        return [4, this.getReplyValues(tag, wordCount, data)];
                    case 3:
                        words = _b.sent();
                        vals = this.wordsToBits(tag, words, elements);
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        if (datatype === 160) {
                            tmp = unpackFrom("<h", data, true, 52)[0];
                            if (tmp == this.context.structIdentifier) {
                                index = 54 + counter * dataSize;
                                NameLength = unpackFrom("<L", data, true, index)[0];
                            ...
                            index = 52 + counter * dataSize;
                            NameLength = unpackFrom("<B", data, true, index)[0];
                            s = data.slice(index + 1, index + 1 + NameLength);
                            vals.push(s.toString("utf8"));
                        }
                        else {
                            returnvalue = unpackFrom(CIPFormat, data, false, index)[0];
                    ...
                    case 3:
                        data_3 = (_a.sent());
                        status = unpackFrom("<B", data_3, true, 48)[0];
    ...
    EIPSocket.prototype.wordsToBits = function (tag, value, count) {
        ...
        var _a = parseTagName(tag, 0), _ = _a[0], basetag = _a[1], index = _a[2], datatype = this.context.knownTags[basetag][0], bitCount = this.context.CIPTypes[datatype][0] * 8;
        var bitPos;
        if (datatype == 211) {
            bitPos = index % 32;
        }
    ...
    EIPSocket.prototype.multiParser = function (tags, data) {
        ...
        for (var i = 0; i < tags.length; i++) {
            ...
            if (Array.isArray(tag))
                tag = tag[0];
            var loc = 2 + i * 2;
            var offset = unpackFrom("<H", stripped, true, loc)[0];
            var replyStatus = unpackFrom("<b", stripped, true, offset + 2)[0];
            var replyExtended = unpackFrom("<b", stripped, true, offset + 3)[0];
            var response = void 0;
            if (replyStatus == 0 && replyExtended == 0) {
                var dataTypeValue = unpackFrom("<B", stripped, true, offset + 4)[0];
                if (BitofWord(tag)) {
                    var dataTypeFormat = this.context.CIPTypes[dataTypeValue][2];
                    var val = unpackFrom(dataTypeFormat, stripped, true, offset + 6)[0];
                    var bitState = getBitOfWord(tag, val);
                    ...
                    var strlen = unpackFrom("<B", stripped, true, offset + 8)[0];
                    ...
                    var dataTypeFormat = this.context.CIPTypes[dataTypeValue][2];
                    var value = unpackFrom(dataTypeFormat, stripped, false, offset + 6)[0];
                    response = new LgxResponse(tag, value, replyStatus);
                }
            }
            else {
    ...
    EIPSocket.prototype.extractTagPacket = function (data, programName) {
        ...
        while (packetStart < data.length) {
            var tagLen = unpackFrom("<H", data, true, packetStart + 4)[0];
        ...
        return Bluebird.try(function () {
            ...
            return _this.getBytes(eipHeader).spread(function (status, retData) {
                if (status == 0 || status == 6) {
                    var dataType = unpackFrom("<B", retData, true, 50)[0];
                    var dataLen = unpackFrom("<H", retData, true, 2)[0];
                    _this.context.knownTags[baseTag] = [dataType, dataLen];
                    return true;
                }
                else {
                    var err = getErrorCode(status);
                    if (status === 7) {
    ...
    EIPSocket.prototype.addWriteIOI = function (tagIOI, writeData, dataType) {
        ...
        if (dataType == 160) {
            ...
            TypeCodeLen = 0x02;
            data = pack("<BBHH", dataType, TypeCodeLen, RequestNumberOfElements, writeData.length);
    ...
    EIPSocket.prototype.writeTag = function (tag, value, options) {
        ...
        return Bluebird.try(function () {
            _this.offset = 0;
            var writeData = [];
            var _a = parseTagName(tag, 0), t = _a[0], b = _a[1], i = _a[2];
    ...
    EIPSocket.prototype.readTag = function (tag, options) {
        ...
        return Bluebird.try(function () {
            ...
            return _this._initial_read(b, dt)
                ...
                if (datatype == 211) {
                    tagData = _this.buildTagIOI(tag, true);
                    words = getWordCount(i, elements, bitCount);
    ...
    EIPSocket.prototype.multiReadTag = function (tags) {
        var _this = this;
        return Bluebird.try(function () { return __awaiter(_this, void 0, void 0, function () {
                ...
                switch (_b.label) {
                    ...
                    case 8:
                        ...
                        if (status == 0) {
                            ...
                            err = getErrorCode(status);
                            throw new LogixError("Multi-read failed-" + tags.toString() + " - " + err, status);
                        }
                        return [2];
                }
            });
    ...
    EIPSocket.prototype.getWallClockTime = function (raw) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        AttributePacket = pack("<BBBBBBH1H", 0x03, 0x02, 0x20, 0x8b, 0x24, 0x01, 0x01, 0x0b);
                        eipHeader = this.buildEIPHeader(AttributePacket);
                        return [4, this.getBytes(eipHeader)];
                    case 1:
                        _a = _b.sent(), status = _a[0], retData = _a[1];
                        if (status == 0) {
                            us = Number(unpackFrom("<Q", retData, true, 56)[0].toString());
                            return [2, raw
                                    ? us
    ...
    EIPSocket.prototype.setWallClockTime = function (date) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        AttributePacket = pack("<BBBBBBHHQ", 0x04, 0x02, 0x20, 0x8b, 0x24, 0x01, 0x01, 0x06, date.getTime() * 1000);
                        eipHeader = this.buildEIPHeader(AttributePacket);
                        return [4, this.getBytes(eipHeader)];
                    case 1:
                ...
                switch (_d.label) {
                    ...
                    case 2:
                        ...
                        if (status_1 == 0 || status_1 == 6)
                            ...
                            err = getErrorCode(status_1);
                            throw new LogixError("Failed to get program tag list " + err, status_1);
                        }
                        _d.label = 3;
                    case 3:
                    ...
                    case 4:
                        ...
                        if (status_2 == 0 || status_2 == 6)
                            ...
                            err = getErrorCode(status_2);
                            throw new LogixError("Failed to get program tag list " + err, status_2);
                        }
                        return [3, 3];
                    case 5:
                        _i++;
                ...
                switch (_c.label) {
                    ...
                    case 3:
                        ...
                        if (status_3 == 0 || status_3 == 6)
                            ...
                            err = getErrorCode(status_3);
                            throw new LogixError("Failed to get program tag list " + err, status_3);
                        }
                        return [3, 2];
                    case 4: return [2];
                }
            ...
            return __generator(this, function (_f) {
                switch (_f.label) {
                    ...
                    case 2:
                        temp = _f.sent();
                        data = temp.slice(46);
                        val = unpackFrom("<I", data, true, 10)[0];
                        words = val * 4 - 23;
                        member_count = unpackFrom("<H", data, true, 24)[0];
                    ...
                    case 6:
                        ...
                        split_char = pack("<b", 0x00).toString();
    ...
    EIPSocket.prototype.getTemplateAttribute = function (instance) {
        return __awaiter(this, void 0, Bluebird, function () {
            var _this = this;
            return __generator(this, function (_a) {
                return [2, Bluebird.try(function () { return __awaiter(_this, void 0, void 0, function () {
    ...
    EIPSocket.prototype.buildTemplateAttributes = function (instance) {
        return pack("<BBBBHHHHHHH", 0x03, 0x03, 0x20, 0x6c, 0x25, instance, 0x04, 0x04, 0x03, 0x02, 0x01);
    };
    EIPSocket.prototype.readTemplateService = function (instance, dataLen) {
        return pack("<BBBBHHIH", 0x4c, 0x03, 0x20, 0x6c, 0x25, instance, 0x00, dataLen);
    };
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        return [2, parseIdentityResponse(data, undefined, resp)];
                }
            });
        }); });
    };
    EIPSocket.prototype.getModuleProperties = function (slot) {
        ...
        return Bluebird.try(function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        AttributePacket = pack("<10B", 0x01, 0x02, 0x20, 0x01, 0x24, 0x01, 0x01, 0x00, 0x01, slot);
                    ...
                    case 1:
                        ...
                        status = unpackFrom("<B", retData, true, 46)[0];
                        return [2, status == 0
        ...
        return Bluebird.try(function () { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        return [4, new Bluebird(function (resolve) {
                                _this.once("data", resolve);
                            }).timeout(this.context.timeoutReceive, "timeout-recv-data")];
                    case 1: return [2, _a.sent()];
                    case 2:
                        error_1 = _a.sent();
                        if (error_1 instanceof Bluebird.TimeoutError)
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 2:
                        ...
                        this.pause();
                        if (retData) {
                            return [2, [unpackFrom("<B", retData, true, 48)[0], retData]];
                        }
                        else {
                            throw [1, undefined];
                        }
    ...
    EIPSocket.prototype.connect = function () {
        ...
        return new Bluebird(function (resolve, reject) {
            ...
            var onError = function () {
                ...
                reject(new ConnectionTimeoutError(_this.context.host, _this.context.connectTimeout));
            };
            ...
            return Bluebird.try(function () {
                if (retData) {
                    _this.sequenceCounter = 1;
                    _this.sessionHandle = unpackFrom("<I", retData, false, 4)[0];
                    return true;
                }
                else {
            ...
            .then(function () { return _this.recv_data(); })
            .then(function (retData) { return [retData, unpackFrom("<b", retData, false, 42)[0]]; })
            ...
            if (!sts) {
                ...
                _this.id = unpackFrom("<I", retData, true, 44)[0];
    ...
    EIPSocket.prototype.destroy = function () {
        var _this = this;
        return Bluebird.resolve(function () { return __awaiter(_this, void 0, void 0, function () {
            var _a;
            return __generator(this, function (_b) {
                switch (_b.label) {
    ...
    EIPSocket.buildListIdentity = function () {
        return pack("<HHIIHHHHI", 0x63, 0x00, 0x00, 0x00, 0xfa, 0x6948, 0x6f4d, 0x006d, 0x00);
    };
...
export default EIPSocket;
function parseLgxTag(packet, programName) {
    var t = new LgxTag();
    var length = unpackFrom("<H", packet, true, 4)[0];
...
var LgxResponse = (function () {
    function LgxResponse(tag_name, value, status) {
        ...
        this.status = status;
        if (status)
            this.message = getErrorCode(status);
    }
